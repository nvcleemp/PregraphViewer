 ...
 * @author nvcleemp
 */
public class Embedding2D extends Embedding<Coordinates2D>{

    public Embedding2D(Pregraph pregraph) {
        super(pregraph);
        for (Vertex v : pregraph.getVertices()) {
            coordMap.put(v, new Coordinates2D());
        }
    }

    @Override
    public Coordinates2D getCoordinates(Vertex v) {
        return coordMap.get(v);
    }
